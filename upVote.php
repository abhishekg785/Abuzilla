...
$uid=$res['userid'];
mysql_query("delete from ans_status where ansId=\"$ansId\"  and userId=\"$uid\" and status=\"downVote\"");
$qt=mysql_query("select *  from ans_status where ansId=\"$ansId\"  and userId=\"$uid\" and status=\"upVote\"");
if(!mysql_num_rows($qt)){
mysql_query("insert into ans_status values('','$ansId','$uid','upVote')");
}
